...
class Chrome extends EventEmitter {
    ...
    async _start() {
        ...
        try {
            ...
            // via the 'error' event
            process.nextTick(() => {
                this._notifier && this._notifier.emit('connect', this);
            });
        } catch (err) {
            this._notifier && this._notifier.emit('error', err);
        }
    }
    ...
    _connectToWebSocket() {
        return new Promise((fulfill, reject) => {
            ...
            this._ws.on('message', (data) => {
                ...
                this._clientWs.send(data.toString())
            });
